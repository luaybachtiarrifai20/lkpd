  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      if (isLogin) {
        ...
        console.log(
          "[AuthPage] refreshAuth returned profile:",
          prof?.role,
          prof?.status,
        );
        ...
        try {
          if (role === "siswa") {
            const kelasSnapshot = await getDocs(
              query(
                collection(db, "kelas"),
                where("kode_undangan", "==", kodeKelas.trim()),
              ),
            );
        ...
        if (status === "pending") {
          ...
          await refreshAuth();
          toast(`Selamat datang, ${nama}! Akun langsung aktif.`, "success");
        ...
        <div className="card animate-fade-in">
          <div className="mb-5 text-center">
            ...
            <h1 className="text-xl font-bold text-slate-800">
              {isLogin ? "Masuk ke LajuNalar" : "Buat Akun Baru"}
            </h1>
            <p className="mt-1 text-sm text-slate-500">
              {isLogin
                ? "Gunakan email & kata sandi akunmu"
                : "Lengkapi data di bawah untuk mulai belajar"}
            </p>
          </div>
            ...
            <div className="mb-4">
              ...
              <div className="grid grid-cols-2 gap-2">
                <RoleBtn
                  ...
                  label="Siswa"
                  color="text-student"
                  activeCls="border-student bg-blue-50"
                />
                <RoleBtn
                  active={role === "guru"}
                  ...
                  label="Guru"
                  color="text-teacher"
                  activeCls="border-teacher bg-violet-50"
                />
              </div>
            </div>
          )}
            ...
            {!isLogin && role === "siswa" && (
              <div>
                ...
                <input
                  ...
                  required
                  placeholder="Kode undangan dari guru"
                />
                <p className="mt-1 text-xs text-slate-400">
                  Wajib diisi. Gunakan kode yang diberikan guru agar akun
                  langsung aktif.
                </p>
              </div>
            )}
