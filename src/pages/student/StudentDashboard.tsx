...
import {
  ...
  BookOpen, KeyRound, Loader2, LogIn, ArrowLeft,
} from 'lucide-react';
import { DashboardLayout } from '@/components/layout/DashboardLayout';
import { useAuth } from '@/context/AuthContext';
import { useToast } from '@/context/ToastContext';
import { KEGIATAN_CONTENT } from '@/content/kegiatanContent';
import { db, type Jawaban, type StatusKuisSiswa, type Kelas } from '@/lib/firebase';
...
export function ProgresKeseluruhan({ rows, loading, kelas, onBack }: ProgresKeseluruhanProps) {
  const statusMeta = (status?: Jawaban['status']) => {
    switch (status) {
      case 'terkumpul':
        return { label: 'Terkumpul', cls: 'bg-success/10 text-success', icon: <CheckCircle2 className="h-3.5 w-3.5" /> };
      case 'dinilai':
        return { label: 'Dinilai', cls: 'bg-brand-teal-light text-brand-teal-dark', icon: <Award className="h-3.5 w-3.5" /> };
      case 'draft':
        return { label: 'Draft', cls: 'bg-brand-amber-light text-[#B26A00]', icon: <FileEdit className="h-3.5 w-3.5" /> };
      default:
        return { label: 'Belum dikerjakan', cls: 'bg-slate-100 text-slate-500', icon: <Clock className="h-3.5 w-3.5" /> };
    }
  };

  const totalDone = Object.values(rows).filter(
    (r) => r.jawaban?.status === 'terkumpul' || r.jawaban?.status === 'dinilai'
  ).length;
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-bold text-slate-800">Progres Keseluruhan</h1>
          {kelas && (
            <p className="text-sm text-slate-500">
              Kelas: <span className="font-semibold text-brand-green">{kelas.nama_kelas}</span>
            </p>
          )}
        </div>
      </div>
                  ...
                  <div className="relative z-10 flex flex-col items-center pt-5">
                    <div
                      ...
                      style={{
                        ...
                        boxShadow: isDone ? `0 0 0 4px ${k.warna}25` : undefined,
                      }}
                    >
    ...
    (async () => {
      ...
      try {
        const [jSnapshot, kSnapshot] = await Promise.all([
          getDocs(query(collection(db, 'jawaban'), where('siswa_id', '==', profile.id))),
          getDocs(query(collection(db, 'status_kuis_siswa'), where('siswa_id', '==', profile.id))),
        ]);
        if (!active) return;
        const jByKeg = jSnapshot.docs.reduce<Record<string, Jawaban>>((acc, d) => {
          const data = { id: d.id, ...d.data() } as Jawaban;
          acc[data.kegiatan_id] = data;
          return acc;
        }, {});
        const kByKeg = kSnapshot.docs.reduce<Record<string, StatusKuisSiswa>>((acc, d) => {
        ...
        kegsSnapshot.docs.forEach((d) => {
          ...
          newRows[k.nomor] = { jawaban: jByKeg[d.id], kuis: kByKeg[d.id] };
        });
        setRows(newRows);
      } catch (err) {
    ...
    try {
      ...
      if (snapshot.empty) {
        snapshot = await getDocs(
          query(collection(db, 'kelas'), where('kode_undangan', '==', kodeInput.trim()))
        );
      }
      ...
      toast(err instanceof Error ? err.message : 'Gagal bergabung ke kelas', 'error');
    } finally {
      setJoining(false);
    }
  };
    ...
    <DashboardLayout items={navItems} role="siswa">
      <div className="space-y-6">
        <div>
          ...
          <p className="text-sm text-slate-500">
            Pilih kelas untuk melihat progres kegiatan, atau gabung dengan kode undangan.
          </p>
        </div>
            ...
            >
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-sm font-bold text-slate-800">{kelas.nama_kelas}</p>
        ...
        {!kelas && (
          <div className="card border-2 border-dashed border-brand-amber/40 bg-brand-amber-light/30">
            <div className="mb-3 flex items-center gap-2">
              ...
              <h2 className="text-sm font-bold text-slate-800">Gabung ke Kelas</h2>
            </div>
            <p className="mb-3 text-xs text-slate-600">
              Masukkan <strong>kode undangan</strong> dari guru Anda. Setelah bergabung, kelas
              muncul di list di atas — klik untuk masuk ke progres.
            </p>
            <form onSubmit={joinKelas} className="flex flex-col gap-2 sm:flex-row sm:items-end">
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!active) return;
        const jByKeg = jSnapshot.docs.reduce<Record<string, Jawaban>>((a, d) => {
          const data = { id: d.id, ...d.data() } as Jawaban;
          a[data.kegiatan_id] = data;
          return a;
        }, {});
        const kByKeg = kSnapshot.docs.reduce<Record<string, StatusKuisSiswa>>((a, d) => {
          const data = { id: d.id, ...d.data() } as StatusKuisSiswa;
          a[data.kegiatan_id] = data;
          return a;
        }, {});
              ...
              return (
                ...
                >
                  <div>
                    ...
                    <p className="text-xs text-slate-500">
                      Jawaban: {item.jawaban?.status || 'Belum dikerjakan'} • Kuis:{' '}
                      {item.kuis?.sudah_mengerjakan ? 'Selesai' : 'Belum'}
                    </p>
                  </div>
                  <div className="flex items-center gap-3">
                    {item.jawaban?.skor != null && (
                      <Badge color="teal">
                        <Award className="h-3.5 w-3.5" /> Skor: {item.jawaban.skor}
                      </Badge>
                    )}
                    {item.kuis?.skor_manual != null && (
                      <Badge color="amber">
                        <Award className="h-3.5 w-3.5" /> Kuis: {item.kuis.skor_manual}
                      </Badge>
                    )}
                    <Link to={`/siswa/kegiatan/${item.nomor}`} className="btn-outline">
                      Lihat
                    </Link>
                  </div>
                </div>
...
export function StudentProfil() {
  ...
  const joinKelas = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setKodeInput('');
      toast(`Berhasil bergabung ke kelas "${kelasDoc.data().nama_kelas}"`, 'success');
    } catch (err) {
      console.error(err);
          ...
          {kelas ? (
            <div className="rounded-xl bg-brand-green-light/40 px-4 py-3">
              ...
              <p className="mt-2 text-xs text-success flex items-center gap-1">
                <CheckCircle2 className="h-3.5 w-3.5" /> Sudah bergabung — tidak perlu kode lagi
              </p>
            </div>
          ) : (
